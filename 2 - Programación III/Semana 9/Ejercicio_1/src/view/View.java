...
public class View {
    ...
    private void importBooksFromCSV() {c.importBooksFromCSV();}
    private void exportarBooksCSV() {c.exportBooksToCSV();}


    private void importarBooksJSON() {c.importBooksFromJSON();}
    private void exportarBooksJSON() {c.exportBooksToJSON();}

    private void importarBooksXML() {c.importBooksFromXML();}
    private void exportBooksToXML() {c.exportBooksToXML();}
    ...
    private void editarLibroDelModelo() {
        ...
        if (libros.isEmpty()) {
            System.out.println("[AVISO : EDITAR LIBRO] No hay libros para editar.");
            return;
        }

        mostrarLibrosModelo(true);
        int index = Esdia.readInt("Seleccione un libro: ", 1, libros.size());
        
        Libro l = libros.get(index - 1);
        ...
        String nuevoAutor = Esdia.readString("Nuevo autor: ");
        if (!nuevoAutor.isEmpty()) l.setAutor(nuevoAutor);

        float nuevoPrecio = Esdia.readFloat("Nuevo precio: ");
    ...
    private void borrarLibroDelModelo() {ArrayList<Libro> libros = c.getLibros();
        
        if (libros.isEmpty()) {
            System.out.println("[AVISO : BORRAR LIBRO] No hay libros para borrar.");
            return;
        }

        mostrarLibrosModelo(true);
        int index = Esdia.readInt("Introduzca el número del libro a borrar: ", 1, libros.size());
        Libro libroABorrar = libros.get(index - 1);
